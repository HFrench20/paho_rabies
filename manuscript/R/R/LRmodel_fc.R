...
LRmodel  <-  function(data){
  ...
  ### input: counts = presence/absence data of cases reports (response variable),
...
LRincursions  <-  function(mydata, method="LR", computeCI=F){
  ...
  ###         variable = period (time autocirrelation) 
  ### output: p-value(s) of variable(s), fitted values,
  ###         LR => chi-square probability, odds ratio, confidence interval of variable, 
  ###               pseudo R-squared 
  ###         RE => 
  ###         TA => phi
...
#                      family = binomial(link="logit")) #try to rescale like this - effect of extra 100 cases the probability of getting an incursion
    ...
    maxgrad <- max(abs(relgrad)) # gradient of likelihood profile; if <0.001 than it is ok 
    ...
    m <- list(chsq.prob = chsq.prob, aic=aic, odds = odds,
              pval = pval, maxgrad=maxgrad) 
    }
