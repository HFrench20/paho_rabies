...
LRmodel  <-  function(data){
  ...
  # response variable = counts. predictor = months. (~ means modelled by)
  # data frame data must have columns that correspond to months and columns
  # and of the family of models I am using (GLMs), choose binomial. Link function = logit (logistic regr with 1's and 0's)
  # ?glm helpfile
  #(counts~months is the formula, family = choose model,)
  # control argument = to help make sure the model doesn't run forever.
  # model uses an optimisation algorithm to come up with the best fitting parameters.
  # it gives parameters starting values and then repeats with closer and closer values.
  # the maximum iterations is 100. if it runs to 100 and hasn't converged I will get a warning.
  # this is quite a simple model so it should converge within 100 iterations.
  model  <-  glm(counts~months, data=data, family=binomial(link="logit"), control = list(maxit = 100))

  # model is now an object that has attributes I can access with $
  # I can explore my model object after running on "states"
  # Itsy does this below, pulling out different aspects of the model to compare to null. 
  # she saves them as new values
  ...
  ## coefficients
  # e.g tell you whether the trend in the presence or absence of rabies is pos. neg. or there is no trend.
  # to interpret a logistic regression, you have to exponentiate it. 
  # only interested in the slope (not intercept) = gives odds ratio? 
  ...
  # pR2 is the pseudo R squared value to extract McFadden pseudo R squared value. 
  pseudoR2 <- pR2(model)["McFadden"]  # pseudo R-squared: 'McFadden' btw 0 (no predictive power) and 1

  # if the data falls perfectly where the model is we will see the fitted values
  fitted <- model$fitted # fitted values

  # all these values that she has extracted from the model are saved in m.
