...
#source("R/RemoveWildlifeVar.R") # function to remove dog cases that are actually wildlife based on 
# available sequence data for Mexico and Brazil. (Not available for most countries).

dogs <- read.csv("~/Code/paho_rabies_hf/manuscript/data/SIRVERA_dogs16_ISO_GID1_GID2.csv")
countries <- c(unique(dogs$ADM0_ISO))

# initialise vectors for storing output feedback
timeseries.exists <- vector()
timeseries.not.exists <- vector()
...
for (l in 1:length(countries)){
  ...
  # import timeseries data - normally in the RemoveWildlifeVar.R function
  fstates <- paste0("output_new/", cn, "_ADM1_monthly_cases.csv")
  
  if (file.exists(fstates)) { 
    states <- read.csv(fstates) # monthly detected cases by state
   
    print(paste("Time series 1995-2015 exists for", cn))
    timeseries.exists <- c(timeseries.exists, cn)
    ...
    for (r in 1:study.mths){ #for each month in r
      ...
      for (i in 1:n){ #for each state in n
        ...
        # for every state, do a logistic regression model and save into m
        # extract the chi quared prob, the odds ratio and the pval.
        
        ## only fit if at least 2 cases and 1 zeros!
        if(sum(state$counts)>1 && sum(state$counts)!=length(state$counts)){
          m = LRmodel(data <- state) ## LR model # save output of model into object m
          ...
          rm(m) # because we are repeating for each state in turn and not all of them will actually fit.
        }
      }
    ...
    for(j in 1:length(LRcoefsList)){
      ...
      write.csv(this.list, paste0("output_script2/", cn, "_MonthlyOutputFor_", this.month, ".csv"), row.names = FALSE)
    }
  } else {
    timeseries.not.exists <- c(timeseries.not.exists, cn)
    print(paste("Time series 1995-2015 does NOT exist for", cn))
  }
}

print("Time series 1995-2015 does exist for:")
print(timeseries.exists)

print("Time series 1995-2015 does NOT exist for:")
print(timeseries.not.exists)


# the core of it is lines 94 to 99. 
